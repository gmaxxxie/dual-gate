...
| `/dual project <request>` | 生成项目 WBS，展示并显式确认后，按依赖分批执行里程碑（无依赖者并行 worktree + 自动合并） |
...
本插件用于**需要明确验收契约、可审计实现闭环的交付型研发工作**，不用于普通对话或极小、低风险改动。

| 需求层次 | 入口 | 适用场景 |
|---|---|---|
| 对话 / 探索层 | 不启动 | 只需问答、调研、头脑风暴或决策，不涉及需要验收证据的仓库变更。 |
| 单任务工程层 | `/dual on`，随后正常发送任务 | 一个 Executor 会话可承接的完整变更，且需经测试、Gate、Judge 验收；如缺陷修复、聚焦功能、重构或安全敏感变更。 |
| 项目 / 产品交付层 | `/dual on`，再执行 `/dual project <需求>` | 需要产品规划、WBS/里程碑、依赖管理、分阶段验收和进度反馈；这才使用三 pane 模式。 |

不要把项目模式用于微小改动、彼此独立的杂项，或不值得里程碑规划与额外模型会话的工作；若项目可以安全地缩为一个验收契约，应优先使用单任务工程层。
